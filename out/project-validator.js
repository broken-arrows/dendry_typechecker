        ...
        try {
            ...
            for (const node of ast.nodes) {
                ...
                if (id) {
                    if (seenIds.has(id)) {
                        ...
                        const sceneDeclarationRange = new vscode.Range(node.range.start.line, 0, node.range.start.line, lines[node.range.start.line]?.length || 0);
                        diagnostics.push(new vscode.Diagnostic(sceneDeclarationRange, `Duplicate ID "${id}" found in this file (first defined on line ${(idToLine.get(id) || 0) + 1})`, vscode.DiagnosticSeverity.Error));
                    }
                    if (!seenIds.has(id)) {
                        seenIds.add(id);
            ...
            if (seenIds.has('jumpScene') || seenIds.has("backSpecialScene") || seenIds.has("backScene")) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (matchBackScene) {
                        const reservedIdRange = new vscode.Range(i, 0, i, lines[i].length);
                        diagnostics.push(new vscode.Diagnostic(reservedIdRange, `"backScene" is a reserved scene ID and cannot be used. Choose a different ID.`, vscode.DiagnosticSeverity.Error));
                        break;
                    }
                }
                // Also check in nodes
    ...
    async validateProject(workspaceFiles, changedFileUri) {
        ...
        try {
            ...
            for (const [fileUri, data] of this.fileData) {
                ...
                if (fileName && fileName !== fullFileName) { // Only check .scene.dry files
                    ...
                    if (existingUri && existingUri.toString() !== fileUri.toString()) {
                        ...
                        currentDiags.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), `Duplicate file name "${fileName}.scene.dry". Another file with this name exists at ${existingUri.fsPath}`, vscode.DiagnosticSeverity.Error));
                        ...
                        existingDiags.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), `Duplicate file name "${fileName}.scene.dry". Another file with this name exists at ${fileUri.fsPath}`, vscode.DiagnosticSeverity.Error));
            ...
            for (const [fileUri, data] of this.fileData) {
                for (const id of data.localQualityIds) {
                    ...
                    if (existingUri && existingUri.toString() !== fileUri.toString()) {
                        ...
                        if (existingRange) {
                            existingDiags.push(new vscode.Diagnostic(existingRange, `Duplicate quality ID "${id}" also found in ${fileUri.fsPath.split(/[/\\]/).pop()}`, vscode.DiagnosticSeverity.Error));
                            finalDiagnostics.set(existingUri, existingDiags);
                        }
                        ...
                        if (currentRange) {
                            currentDiags.push(new vscode.Diagnostic(currentRange, `Duplicate quality ID "${id}" also found in ${existingUri.fsPath.split(/[/\\]/).pop()}`, vscode.DiagnosticSeverity.Error));
                            finalDiagnostics.set(fileUri, currentDiags);
                        }
                    }
                    else {
            ...
            for (const [fileUri, data] of this.fileData) {
                ...
                const hasParsingErrors = existingDiags.some(diag => diag.message.includes('Lexer Error:') ||
